...
| --- | --- | --- |
| [Threat Detection Suite](https://github.com/genesisgzdev/threat-detection-suite) | Ingeniería de detección para Windows con C/C++ y capas nativas | El ABI, los callbacks de proceso/imagen/hilo, las dos colas acotadas y sus contadores tienen checks de contrato. Las respuestas usan el PID objetivo cuando el evento lo entrega; la validación nativa de driver y ETW sigue siendo una etapa separada. |
| [Project MegaTicketing](https://github.com/genesisgzdev/Project-MegaTicketing) | Reservas concurrentes con Fastify, PostgreSQL, Redis, Stripe y React | PostgreSQL decide la venta, el outbox conserva eventos y el ticket guarda la asociación de importe, moneda y PaymentIntent. Los webhooks repetidos no vuelven a entrar al camino de refund. La integración real requiere DB, Redis y Stripe. |
